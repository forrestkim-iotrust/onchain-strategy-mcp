...
pub(crate) async fn list_resources_impl(
    ...
    // Phase 2: stay empty for enumerable surfaces (strategies, runs, etc.) —
    // those duplicate `strategy_list` and friends.
    //
    // v1.11 Track B: surface the runtime plane. `runtime://status` is a
    // singleton URI (no parameters), so it belongs in `list_resources` rather
    // than `list_resource_templates`. The wider catalogue rewrite lives in
    // Track I; we drop this single entry so the namespace is discoverable.
    let runtime_status = Annotated::new(
        RawResource::new("runtime://status", "runtime-status")
            .with_description(
                "v1.11 runtime plane: snapshot of operational state — chain_id, burner, \
                 RPC reachability, watcher health, active trigger count, and the last-24h \
                 run rollup. All probes are wrapped in 500ms timeouts; degraded paths \
                 downgrade `confidence` to `partial` or `missing` with a non-empty `reason`.",
            )
