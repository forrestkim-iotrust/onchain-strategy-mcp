...
use executor_mcp::tools::execute_approved_actions;
...
fn seed_run(store: &mut StateStore) -> String {
    ...
    store
        .insert_run(&strategy_id, executor_core::schema::execution::RunStatus::Running)
        .expect("insert run")
}
...
async fn execution_actions_signer_not_configured_records_execution_action_error() {
    ...
    assert_eq!(executions[0].error_kind.as_deref(), Some("signer_not_configured"));
}
