...
- **Out-of-band 알림.** strategy 안에서 쓰는 `ctx.notify({channel, message})` API와 Telegram, Discord, ntfy.sh, generic 웹훅에 대한 1급 어댑터. 트리거는 어차피 백그라운드에서 발화하지만, 지금은 strategy가 로컬 journal에만 적습니다. 이게 들어가면 strategy가 *유저를* 깨움 — 다음에 물어볼 때까지 묻혀 있는 게 아니라.
